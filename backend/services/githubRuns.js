 ...
 *   'unavailable' — the token cannot read Actions (403, or the 404 GitHub
 *   returns instead of 403 on a private repo); asking again will not help.
 *   'pending'     — transient (429/5xx); the caller should retry later.
 */
const listRunsSince = async (workflowFile, since) => {
  const url = new URL(`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/actions/workflows/${encodeURIComponent(workflowFile)}/runs`);
  ...
  const response = await fetch(url, {
    headers: { ...ghHeaders(), 'User-Agent': 'dfcci-threshold' },
